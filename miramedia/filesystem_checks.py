...
def run_filesystem_checks(config: MiraMediaConfig, log: Logger) -> None:
    ...
    test_dir.rmdir()
    log.info("Successfully created test dir in Show directory at: %s", test_dir)
    test_dir = config.misc.movie_directory / Path(".miramedia_test_dir")
    test_dir.mkdir(parents=True, exist_ok=True)
    test_dir.rmdir()
    log.info("Successfully created test dir in Movie directory at: %s", test_dir)
    test_dir = config.misc.image_directory / Path(".miramedia_test_dir")
    test_dir.touch()
    test_dir.unlink()
    log.info("Successfully created test file in Image directory at: %s", test_dir)
    test_dir = config.misc.show_directory / Path(".miramedia_test_dir")
    test_dir.mkdir(parents=True, exist_ok=True)
