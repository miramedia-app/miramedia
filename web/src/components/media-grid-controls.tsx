...
import { TOOLBAR_SEARCH_SLOT_CLASS } from "@/components/data-list/data-list-toolbar";
...
export function MediaGridControls<T = any>({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className={TOOLBAR_SEARCH_SLOT_CLASS}>
        {useFacets ? (
          ...
          <div className="flex h-8 min-w-0 flex-1 items-center gap-1.5 rounded-md border border-input bg-background px-2.5 text-sm shadow-xs transition-colors focus-within:border-ring focus-within:ring-2 focus-within:ring-ring/50 coarse:h-11">
        ...
        <DropdownMenuTrigger
          render={
            <Button variant="outline" size="default" className="gap-1 text-xs">
              ...
              <span className="max-sm:sr-only">{sortLabel}</span>
            </Button>
          }
        />
      ...
      <Button
        size="default"
        className="ml-auto gap-1 text-xs sm:ml-0"
        render={<Link href={addHref} aria-label={addLabel} />}
      >
        <Plus className="h-4 w-4" />
        <span className="max-sm:sr-only">{addLabel}</span>
      </Button>
    </div>
  );
}
