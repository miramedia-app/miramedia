...
export default function MoviesAddPage() {
    ...
    <>
      ...
      <main className="flex w-full flex-1 flex-col gap-4 p-4 pt-0">
        ...
        {errorMessage ? (
          ...
          <h3 className="mx-auto text-muted-foreground">No movies found.</h3>
        ) : results ? (
          <>
            <div className="grid w-full gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {results.map((result) => (
                <AddMediaCard key={String(result.external_id)} result={result} isShow={false} />
              ))}
            </div>
            {hasMore && isLoading && (
